...
let testImage () =
  let imageSpec =
    FakeVar.getOrFail<ImageSpecItem> BuildParams
  ...
  if imageSpec.SkipTest then
    ...
    let checkVersion item itemValue (actualValue: string) =
      Trace.tracefn $"Q: Is %s{item}'s version %s{itemValue} ?"

      if itemValue.Trim() <> actualValue.Trim()
         && not
            <| inAcceptList (item, itemValue.Trim(), actualValue.Trim()) then
        Trace.traceErrorfn $"A: No, %s{item}'s version is %s{actualValue}!"
    ...
    let dotnetVersion =
      if imageSpec.Sdk then
        dockerRunRm [ "dotnet"; "--version" ]
      else
        let reg =
          Regex("^  Microsoft.AspNetCore.App (?<runtime>.*) \\[.*\\]$", RegexOptions.Multiline)

        let result =
          dockerRunRm [ "dotnet"; "--info" ]

        (reg.Match(result).Groups.Item "runtime").Value

    dockerRunRm [ "node"; "--version" ] |> ignore
    dockerRunRm [ "yarn"; "--version" ] |> ignore
    dockerRunRm [ "volta"; "--version" ] |> ignore
